...
export default function Notifications() {
  ...
  useEffect(() => { if (!open) return; const id = setInterval(fetchItems, 60000); return () => clearInterval(id); }, [open]);
  useEffect(() => {
  ...
  return (
    <div className="notif-wrapper" ref={dropdownRef} style={{ position: 'relative' }}>
      <button className="notif-bell" onClick={() => setOpen(!open)} title="Powiadomienia" aria-label="Powiadomienia">
        ...
        {count > 0 && <span className="notif-badge">{badgeText}</span>}
      </button>
      {open && (
        <div className="notif-dropdown">
          ...
          <div className="notif-list">
            ...
            {!loading && items.length === 0 && <div className="notif-empty">Brak nowych aktywności</div>}
